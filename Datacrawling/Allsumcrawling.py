...
everythingfile = "./everything.json"
...
def PageCrawler(pk, recipeUrl):
    ...
    if len(recipe_intro) < 4:
        print('{} : {}'.format(recipe_id, 'recipe_intro Error'))
        return
    
    if len(recipe_intro) == 4:
        if recipe_intro[1] == '이상' and recipe_intro[3] == '이내':
            print('{} : {}'.format(recipe_id, 'recipe_intro Error'))
            return
        ...
        for n in res.find_all('ul'):
            ...
            for tmp in n.find_all('li'):
                ...
                if data in ingre_justname:
                    ...
                    everything.append(Ingredata)
                    everything.append(orderIngredata)
    ...
    try:
        for n in res.find_all('div', 'view_step_cont'):
            ...
            everything.append(orderImgContentdata)
...
# next_num = 6943045 
# stop_point = 6864418
for i in range(6828809, 6943045+1):
    # flag = 0
    recipe_id = str(i)
...
with open(everythingfile, 'w', encoding="utf-8") as file:
    json.dump(everything, file, ensure_ascii=False, indent="\t")
