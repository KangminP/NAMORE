...
import "./MyDislikeIngre.css";

import Chip from "@material-ui/core/Chip";
import Paper from "@material-ui/core/Paper";

class MyDislikeIngre extends Component {
  constructor() {
    super();
    this.state = {
      options: [],
  ...
  componentDidMount() {
    ...
    //유저 기피 재료 정보 요청(재료 이름만 받음)
    const token = JSON.parse(sessionStorage.getItem("userToken"));
    axios
      .get("http://j3d102.p.ssafy.io/api/auth/hate/", {
        // .get("http://localhost:8000/api/auth/hate/", {
        headers: {
      ...
      .then((res) => {
        sessionStorage.setItem("hateIngre", JSON.stringify(res.data));
        const hateIngre = JSON.parse(sessionStorage.getItem("hateIngre"));
        const ingre = JSON.parse(localStorage.getItem("ingre"));

        // 유저가 선택한 재료 이름을 전체 데이터와 대조해서 보유한 재료 id값 찾아오는 로직
        var userSelected = [];
        for (var k of hateIngre.values()) {
        ...
        // 유저가 기피하는 재료id와 재료이름을 Select 컴포넌트에서 활용할 수 있는 형식으로 가공 후 state에 저장
        var toState = [];
  ...
  onSubmit = () => {
    ...
    axios
      .post("http://j3d102.p.ssafy.io/api/auth/hate/", ingreForm, {
        // .post("http://localhost:8000/api/auth/hate/", ingreForm, {
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json;charset=UTF-8",
          Authorization: `Token ${token}`,
                    ...
                    <div className="row d-flex justify-content-between align-items-center tag-bar">
                      <div className="col-sm-12 col-md-9 h-100 d-flex flex-start">
                        <h5 className="chipPaperHeaderTitle">
                          <i className="mx-2 fas fa-thumbs-down"></i> 내가
                          싫어하는 재료
                        </h5>
                      </div>
